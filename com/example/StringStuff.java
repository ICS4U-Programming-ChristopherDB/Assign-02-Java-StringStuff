  ...
  private static String shrink(final String str) {
    ...
    for (int i = 0; i < sortedString.length; i++) {
      ...
      if (sortedString[i] == previousChar) {
         ...
         */
        if (repetitions != 1) {
          String repCount = Integer.toString(repetitions - 1);
          shrinkString += repCount + previousChar;
  ...
  private static String blowUp(final String str) {
    ...
    for (int i = 0; i < str.length(); i++) {
      if (Character.isDigit(str.charAt(i))) {
        ...
        /* Gets string value of the selected character. This is done
         * so the repeat method can be called in order to repeat
         * the character however many times needed.
         */
        String currentString = String.valueOf(str.charAt(i + 1));
        String blownUpSegment = currentString.repeat(repetitions);
